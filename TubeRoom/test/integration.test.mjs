...
test('stock WLED groups, mic routing rollback, preset ownership, no DDP, restart persistence and recovery',{timeout:30000},async t=>{
 const root=new URL('../',import.meta.url),data=fs.mkdtempSync(path.join(os.tmpdir(),'tube-native-'));let child,failIP='',micFail=false,calls=[],packets=0,beatSender,streamTimer,slowBeatWrites=false;
 ...
 if(b){if(slowBeatWrites&&b.seg?.[0]?.col&&!('fx' in b.seg[0]))await delay(120);if(b.psave)d.presets[b.psave]={...structuredClone(d.state),n:b.n};else if(b.pdel)delete d.presets[b.pdel];else if(!b.rb)Object.assign(d.state,b);res.end('{"success":true}');return;}
 ...
 t.after(async()=>{clearInterval(streamTimer);if(beatSender)beatSender.close();await close();udp.close();mock.close();fs.rmSync(data,{recursive:true,force:true});});
 ...
 await post('blackout');assert.equal(devices['127.0.0.5'].state.on,false);assert.equal(packets,0);
 // Beat loop: real UDP Audio Sync v2 input, no scheduled color progression.
 s=await get();await post('config',{microphone:0});assert.equal((await post('microphone')).code,200);
 s.config.groups[0].effect='loop';s.config.groups[0].loopTrigger='beat';s.config.groups[0].colors=['#ff0000','#00ff00','#0000ff'];
 s.config.groups[1].effect='loop';s.config.groups[1].loopTrigger='beat';s.config.groups[1].colors=['#ffff00','#ff00ff'];
 await post('config',{groups:s.config.groups});
 // Wrong selected source: traffic from tube 1 must not trigger a tube 4 source.
 const packet=peak=>{const b=Buffer.alloc(44);b.write('00002\0',0,'ascii');b[16]=peak?1:0;return b;};
 beatSender=dgram.createSocket('udp4');await new Promise(r=>beatSender.bind(0,'127.0.0.1',r));
 const send=(p=packet(false))=>new Promise((resolve,reject)=>beatSender.send(p,11988,'127.0.0.1',e=>e?reject(e):resolve()));
 streamTimer=setInterval(()=>send().catch(()=>{}),25);
 await post('config',{microphone:3});await post('microphone');let result=await post('start');assert.equal(result.code,400);assert.match(result.error,/No WLED Audio Sync/);assert.equal((await get()).status.live,false);
 await post('config',{microphone:0});await post('microphone');result=await post('start');assert.equal(result.code,200,JSON.stringify(result));
 const color=ip=>devices[ip].state.seg[0].col[0];
 assert.deepEqual(color('127.0.0.1'),[255,0,0]);assert.deepEqual(color('127.0.0.4'),[255,255,0]);
 const before=calls.length;await delay(500);assert.equal(calls.length,before,'silence cannot advance the loop');assert.deepEqual(Object.keys(devices['127.0.0.1'].presets),['200'],'beat mode writes no presets');
 await send(Buffer.alloc(44,1));await send(Buffer.from('bad'));await delay(50);assert.deepEqual(color('127.0.0.1'),[255,0,0]);
 await send(packet(true));await delay(50);assert.deepEqual(color('127.0.0.1'),[0,255,0]);assert.deepEqual(color('127.0.0.3'),[0,255,0]);assert.deepEqual(color('127.0.0.4'),[255,0,255]);
 await send(packet(true));await delay(50);assert.deepEqual(color('127.0.0.1'),[0,255,0],'duplicate/close triggers do not double advance');
 await delay(150);await send(packet(true));await delay(60);assert.deepEqual(color('127.0.0.1'),[0,0,255]);assert.deepEqual(color('127.0.0.4'),[255,255,0],'groups wrap through independent color counts');
 clearInterval(streamTimer);const silent=calls.length;await delay(2700);assert.equal((await get()).status.beat.connected,false);assert.equal(calls.length,silent,'lost input holds color, no timer fallback');
 streamTimer=setInterval(()=>send().catch(()=>{}),25);await delay(40);assert.equal((await get()).status.beat.connected,true);
 // Stop waits for in-flight color writes before restoring, preventing late relighting.
